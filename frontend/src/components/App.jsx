...
function App() {
  ...
  useEffect(() => {
    if (localStorage.jwt) {
      getUserData(localStorage.jwt)
        ...
        .catch((err) =>
          console.log(`Ошибкак авторизации при повторном входе ${err}`)
        );
    } else {
      setIsCheckToken(false);
      setLoggedIn(false);
  ...
  const handleDeleteSubmit = useCallback(() => {
    function makeRequest() {
      return api.deleteCard(deleteCardId, localStorage.jwt).then(() => {
        setCards(
          cards.filter((card) => {
  ...
  const handleUpdateUser = useCallback(
    (dataUser) => {
      function makeRequest() {
        return api.setUserInfo(dataUser, localStorage.jwt).then((res) => {
          setCurrentUser(res);
        });
      }
  ...
  const handleUpdateAvatar = useCallback(
    (dataUser) => {
      function makeRequest() {
        return api.setNewAvatar(dataUser, localStorage.jwt).then((res) => {
          setCurrentUser(res);
        });
      }
  ...
  const handleAddCard = useCallback(
    (dataCard) => {
      function makeRequest() {
        return api.addCard(dataCard, localStorage.jwt).then((res) => {
          setCards([res, ...cards]);
        });
      }
